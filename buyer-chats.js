...
const state = {
  role: 'buyer',
  userUid: document.body?.dataset?.buyerUid || '',
  userName: document.body?.dataset?.buyerName || 'Buyer',
  chats: [],
  filtered: [],
  unsubscribe: null,
};
...
function createCard(chat) {
  ...
  card.innerHTML = `
    ...
    <div class="chat-meta">
      <span class="time">${escapeHtml(formatTime(chat.lastUpdated))}</span>
...
function render() {
  ...
  state.filtered.forEach((chat) => {
    fragment.appendChild(createCard(chat));
  });
...
function normaliseSnapshot(snapshot) {
  ...
  return snapshot.docs.map((docSnap) => {
    ...
    const counterpartyName = data.vendor_name || 'Marketplace Vendor';
    return {
      chatId: docSnap.id,
      ...
      initials: counterpartyName
        ...
        .slice(0, 2)
        .toUpperCase() || 'YU',
      preview: data.last_message || 'Say hello and ask about availability.',
      lastUpdated,
      typing: Boolean(data.vendor_typing),
      unreadCount: Number(data.unread_for_buyer || 0),
    };
  });
}
...
function init() {
  if (!state.userUid) {
    console.warn('[buyer-chats] Missing buyer uid');
    return;
  }

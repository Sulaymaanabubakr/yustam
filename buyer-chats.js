...
const listShell = document.getElementById('buyerChatList');
const listContainer = document.getElementById('chatList');
const emptyState = document.getElementById('emptyState');

if (!listShell || !listContainer) {
  console.warn('[buyer-chats] Chat list container not found.');
}
...
function createChatCard(chat) {
  ...
  const counterpartyName = chat.vendorName || chat.counterpartyName || 'Vendor';
  const lastMessageText = chat.lastMessage?.text || (chat.lastMessage?.imageUrl ? '📷 Photo' : 'Tap to start chatting');
  ...
  destination.searchParams.set('buyerId', chat.buyerId || userId);
  destination.searchParams.set('vendorId', chat.vendorId || '');
  ...
  destination.searchParams.set('status', 'Last seen recently');
...
function listenToChats() {
  ...
  const q = query(chatsRef, where('buyerId', '==', userId), orderBy('lastUpdated', 'desc'));
